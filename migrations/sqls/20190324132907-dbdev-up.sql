...
CREATE TABLE IF NOT EXISTS person (
  ...
  personType nvarchar(32) NOT NULL, /*PersonType (volunteer,boardmember,client,donor) */
...
CREATE TABLE IF NOT EXISTS contact (
  ...
  primaryContact varchar(32),
  primaryCarrier varchar(32),
  textMessages int, /*Would client like to recieve text messages from CU*/
...
CREATE TABLE IF NOT EXISTS demographics (
  ...
  dateUSArrival datetime, /*date of us arrival, day need not be exact and can be defaulted to 1 or 15*/
  employed varchar(5), /*employed binry Y-U-N*/
  ...
  hhIncome varchar(128), /*Predefined income brackets at front end, not applicable for children*/
...
CREATE TABLE IF NOT EXISTS address (
  ...
  duelingType varchar(32), /*Dueling type, home, appartment, duplex, townhome*/
  owned varchar(5), /*Dueling ownership type, rent or owned(Y/N)*/
  city varchar(64),
  zip varchar(5),
  state varchar(2),
  dateAdded DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS programs (
    ...
    FOREIGN KEY (programManager) REFERENCES person(personId)
);
/*
Create Services Table: Services are in turn adminstered by a program. Pogram 1:M Services
*/
CREATE TABLE IF NOT EXISTS servcies (
...
/*
Create LeadServicesNeeded: When a record is added to leads table it may require more than once serives. This table tracks all services required by each lead.. lead 1:M leadServicesNeeded
*/
CREATE TABLE IF NOT EXISTS leadServicesNeeded (
    ...
    dateAdded DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (sericeId) REFERENCES services(serviceId),
    FOREIGN KEY (leadId) REFERENCES lead(leadId)
);

/*
FROM HERE ON, THE DATABASE IS GETTING INTO MORE DETAIL AND THIS MAYBE PART OF ADDED MODELS.. FOR DISCUSSION
...
CREATE TABLE IF NOT EXISTS intakeData (
    ...
    leadtoCU varchar(64), /*how did person hear of CU, Radi, TV, Newspaper, Friend, N/A current client*/
    programId int NOT NULL,
    dateOfIntake date NOT NULL, /*Not time stap because it maybe different*/
    caseNotes varchar(5000),
    registeredVoter varchar(5), /*Y N*/
    dataAdded DATETIME DEFAULT CURRENT_TIMESTAMP,
    addedBy int NOT NULL,
    FOREIGN KEY (addedby) REFERENCES person(personId),
    FOREIGN KEY (personId) REFERENCES person(personId)
);
